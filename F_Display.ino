...
void mainScreen() {
    ...
    drawText(0,0,4,ST77XX_WHITE,"Principale");
    tft.fillScreen(ST77XX_BLACK);
    updateTime();
    ...
    if (TimeManager::dayOfWeekNum() == 0) { // Domenica
      drawText(10,44,2,ST77XX_RED, TimeManager::dayOfWeekIT().c_str());
    } else {
      drawText(10,44,2,ST77XX_GREEN, TimeManager::dayOfWeekIT().c_str());
    }
    ...
    //drawText(5,68,2,ST77XX_WHITE,data.c_str());*/
    drawText(5,68,2,ST77XX_WHITE,TimeManager::dateDDMMYYYY().c_str());
    ...
    drawText(34, 110, 2, ST77XX_WHITE,
    ...
    // Min. T.
    drawText(249,72,2,ST77XX_CYAN,(String(temperatureMin[currentDay].as<float>())).c_str());

    // Max T.
    drawText(249,100,2,ST77XX_ORANGE,(String(temperatureMax[currentDay].as<float>())).c_str());

    //Icona Meteo
    drawMeteo(256,0,weatherConvert(currentDay).icona,64,64);    

    //orizzontale y1
    tft.drawLine(0, 40, 130, 40, ST77XX_WHITE); 

    //orizzontale y2
    tft.drawLine(0, 85, 240, 85, ST77XX_WHITE); 

    //orizzontale y3
    tft.drawLine(0, 130, 320, 130, ST77XX_WHITE); 

    //orizzontale y4
    tft.drawLine(240, 64, 320, 64, ST77XX_WHITE); 

    //verticale x1
    tft.drawLine(92, 85, 92, 130, ST77XX_WHITE); 

    //verticale x2
    tft.drawLine(130, 0, 130, 85, ST77XX_WHITE); 
     
    //verticale x3
    tft.drawLine(240, 0, 240, 130, ST77XX_WHITE); 

    //verticale x5
    tft.drawLine(220, 130, 220, 170, ST77XX_WHITE); 
}

void setupScreen() {

  tft.fillScreen(ST77XX_BLACK);
  drawText(0,0,4,ST77XX_WHITE,"Setup");
  tft.fillScreen(ST77XX_BLACK);
  ...
  drawText(scenari[scenarioAttivo].xText,scenari[scenarioAttivo].yText,2,ST77XX_GREEN,(scenari[scenarioAttivo].nome).c_str());
  parametriScenario(scenarioAttivo);

  if(connessoMqtt){drawText(1,140,3,ST77XX_GREEN,String(mqtt_nodeID).c_str());
  }else{drawText(1,140,3,ST77XX_RED,String(mqtt_nodeID).c_str());
  }
  drawText(223,145,2,ST77XX_WHITE,"ver");
  drawText(263,145,2,ST77XX_WHITE,String(currentVersion).c_str());

  //orizzontale y1
  tft.drawLine(210, 90, 320, 90, ST77XX_WHITE); 
  //orizzontale y2
  tft.drawLine(0, 130, 320, 130, ST77XX_WHITE); 
  //verticale 1
  tft.drawLine(210, 0, 210, 130, ST77XX_WHITE);
  //verticale 2
  tft.drawLine(220, 131, 220, 170, ST77XX_WHITE);  
  //verticale 3
  tft.drawLine(270, 90, 270, 130, ST77XX_WHITE); 

}

void parametriScenario(int indicescenario){  // mostra i parametri dello scenario
  //cancella finestra luminosita
	 tft.fillRect(211, 91, 59, 39, ST77XX_BLACK); //Sfondo x,y punto di partenza; lunghezza, altezza in pixel, colore
	//cancella finestra timer
	 tft.fillRect(271, 91, 50, 39, ST77XX_BLACK); //Sfondo x,y punto di partenza; lunghezza, altezza in pixel, colore
	//cancella finestra slider
	 tft.fillRect(211, 0, 109, 89, ST77XX_BLACK); //Sfondo x,y punto di partenza; lunghezza, altezza in pixel, colore
  
  // Durata complessiva del timer nello scenario attivo
  drawText(280,105,2,ST77XX_WHITE,(String(scenari[indicescenario].timer).c_str()));
  
  // Luminosità interna dello scenario attivo
  drawText(230,105,2,ST77XX_WHITE,(String(scenari[indicescenario].luminosita).c_str()));

  //Slider LED
  //x,y,lenght,height,value,min,max,trackcolor,thumbcolor
  drawSlider(218,10,97,15,scenari[indicescenario].ledWhite,0,100,ST77XX_WHITE,ST77XX_WHITE);  //Led WHITE
  drawSlider(218,35,97,15,scenari[indicescenario].ledRed,0,100,ST77XX_RED,ST77XX_RED);	      //Led RED
  drawSlider(218,60,97,15,scenari[indicescenario].ledBlue,0,100,ST77XX_BLUE,ST77XX_BLUE);	    //Led BLUE
}
...
void weatherScreen() {
  ...
  tft.setTextSize(2);
  if (giorno == "Domenica") {
    drawText(2,14,2,ST77XX_RED,   giorno.c_str());
  } else {
    drawText(2,14,2,ST77XX_GREEN, giorno.c_str());
  }

  // Data "DD-MM-YYYY"
  drawText(118,14,2,ST77XX_WHITE, (String(day + "-" + month + "-" + year)).c_str());

  // Meteo del giorno selezionato
  drawText(1,135,2,ST77XX_WHITE, (weatherConvert(currentDay).decoted).c_str());
  drawMeteo(256,0, weatherConvert(currentDay).icona, 64,64);
  ...
  tft.drawLine(240, 65, 320, 65, ST77XX_WHITE);
  tft.drawLine(0, 80, 240, 80, ST77XX_WHITE);
  tft.drawLine(0, 125, 320, 125, ST77XX_WHITE);
  tft.drawLine(120, 80, 120, 125, ST77XX_WHITE);
  tft.drawLine(240, 0, 240, 65,  ST77XX_WHITE);
  tft.drawLine(240, 0, 240, 125, ST77XX_WHITE);
}
...
void transitionScreen(bool isAlba){
  ...
  lastFillWidth = 0;
  tft.fillScreen(ST77XX_BLACK);
  uint16_t color = isAlba ? ST77XX_YELLOW : ST77XX_ORANGE;
  drawText(0,20,4,color, isAlba ? "Alba" : "Tramonto");
  tft.drawRect(TRANS_BAR_X, TRANS_BAR_Y, TRANS_BAR_W, TRANS_BAR_H, ST77XX_WHITE);
  tft.fillRect(TRANS_BAR_X + 1, TRANS_BAR_Y + 1, TRANS_BAR_W - 2, TRANS_BAR_H - 2, ST77XX_BLACK);
}
  ...
  switch(screen){
    ...
    case 3: //menu Setup con evidenziato scenario selezionato
      ...
      if(oldCurrentIndex!=currentIndex){  //camcella evidenziazione precedente
        tft.setTextColor(ST77XX_WHITE);
        tft.fillRect(scenari[oldCurrentIndex].xText,scenari[oldCurrentIndex].yText, 175, 15, ST77XX_BLACK);
        drawText(scenari[oldCurrentIndex].xText,scenari[oldCurrentIndex].yText,2,ST77XX_WHITE,(scenari[oldCurrentIndex].nome).c_str());
      }
      //sfondo evidenziato
      tft.fillRect(scenari[currentIndex].xText,scenari[currentIndex].yText, 175, 15, ST77XX_WHITE);

      //scrivi testo su evidenziato
      if(scenarioAttivo!=currentIndex){
        drawText(scenari[currentIndex].xText,scenari[currentIndex].yText,2,ST77XX_BLACK,(scenari[currentIndex].nome).c_str());
      }
      ...
      tft.fillRect(221,131,100,39,ST77XX_BLACK);
      //verticale 
      tft.drawLine(270, 131, 270, 170, ST77XX_WHITE); 
      ...
      if(cicloContinuo){
        drawText(280,140,3,ST77XX_GREEN,"CL"); //timer attivo
      }else{
        drawText(280,140,3,ST77XX_RED,"CL");  //timer spento
      }    
    break;
    ...
    case 4:
      ...
      drawText(280,105,3,ST77XX_WHITE,(String(scenari[scenarioAttivo].timer)+"h").c_str());
      drawText(230,105,3,ST77XX_WHITE,String(scenari[scenarioAttivo].luminosita).c_str());
...
void updateTransitionBar(float progress){
  ...
  uint16_t color = isSunriseMode ? ST77XX_YELLOW : ST77XX_ORANGE;
...
void updateTime(){
  ...
  // Ora
  tft.fillRect(0, 0, 130, 40, ST77XX_ORANGE); //Sfondo x,y punto di partenza; lunghezza, altezza in pixel, colore
  drawText(5,5,4,ST77XX_WHITE,TimeManager::timeHHMM().c_str());

  // Temperatura
  tft.fillRect(137, 47, 240-145, 85-55, ST77XX_BLACK); //Sfondo x,y punto di partenza; lunghezza, altezza in pixel, colore
  drawText(140,50,3,ST77XX_WHITE,String(Temperatura).c_str());

  // Umidità
  tft.fillRect(125, 92, 238-128, 130-100, ST77XX_BLACK); //Sfondo x,y punto di partenza; lunghezza, altezza in pixel, colore
  drawText(128,95,3,ST77XX_WHITE,(String(Umidita)+"%").c_str());
  return;
}
...
void weatherList(int currentDay_local){
  ...
  tft.fillScreen(ST77XX_BLACK);
  tft.fillRect(0, 0, 159, 42, ST77XX_MAGENTA);
  drawText(40,10,3,ST77XX_YELLOW,"METEO");
  tft.setTextSize(2);
  tft.setTextColor(ST77XX_WHITE);
  ...
  for (int n = 0; n < daysToShow; n++) {
    ...
    uint16_t color = (n == currentDay_local) ? ST77XX_GREEN : ST77XX_WHITE;
    // Esempio opzionale: oggi in giallo
    // if (TimeManager::dayOfWeekNumFromISO(isoDate) == idxToday) color = ST77XX_YELLOW;
  ...
  // griglia
  tft.drawLine(0, 42, 320, 42, ST77XX_WHITE);
  tft.drawLine(0, 84, 320, 84, ST77XX_WHITE);
  tft.drawLine(0, 126, 320, 126, ST77XX_WHITE);
  tft.drawLine(120, 42, 120, 170, ST77XX_WHITE);
  tft.drawLine(160, 0, 160, 170, ST77XX_WHITE);
  tft.drawLine(280, 0, 280, 1170, ST77XX_WHITE);
}

void drawQRCode(const char* text, int x, int y, int scale, String label) {
  QRCode qrcode;
  tft.fillScreen(ST77XX_WHITE);
  drawText(1,0,3,ST77XX_BLUE,label.c_str());
  ...
  for (uint8_t yy = 0; yy < qrcode.size; yy++) {
    for (uint8_t xx = 0; xx < qrcode.size; xx++) {
      int color = qrcode_getModule(&qrcode, xx, yy) ? ST77XX_BLACK : ST77XX_WHITE;
...
void drawSlider(int x, int y, int length, int height, int value, int minValue, int maxValue, uint16_t trackColor, uint16_t thumbColor) {
  ...
  tft.drawRect(thumbX - thumbSize / 2, thumbY, thumbSize, thumbSize, ST77XX_WHITE); // Bordo opzionale
}
...
void formConferma(String titolo, String testo){
  ...
  tft.fillRect(60, 25, 200, 120, ST77XX_BLACK);
  tft.drawLine(60, 25, 260, 25, ST77XX_WHITE); 
  tft.drawLine(60, 145, 260, 145, ST77XX_WHITE); 
  tft.drawLine(60, 25, 60, 145, ST77XX_WHITE); 
  tft.drawLine(260, 25, 260, 145, ST77XX_WHITE); 
  // Scrivi titolo e Testo     
  drawText(72,29,3,ST77XX_YELLOW,titolo.c_str());
  drawText(67,70,2,ST77XX_YELLOW,testo.c_str());
